...
export default function CreateProjectUpdatePage({ params }: { params: { projectId: string } }) {
  ...
  const fetchProject = async () => {
    try {
      const response = await fetch(`/api/products/${params.projectId}`)
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    console.log('Form submitted with:', { title, content, projectId: params.projectId })
    
    if (!title.trim() || !content.trim() || submitting) return

    setSubmitting(true)
    try {
      console.log('Sending request to /api/updates...')
      ...
      if (response.ok) {
        ...
        console.error('Error creating update:', errorData)
        setErrorMessage(errorData.error || 'Failed to create update. Please try again.')
      ...
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <Button asChild>
              <a href="/updates/new">Back to Projects</a>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground">Create Update</h1>
              <p className="text-muted-foreground">Share updates about {project.name}</p>
            </div>
          </div>
        </div>
            ...
            <div className="text-center space-y-6">
              ...
              <div className="space-y-2">
                <p className="text-lg font-medium">
                  Your update has been created successfully!
                </p>
                <p className="text-muted-foreground">
                  Your community will be notified about this update.
                </p>
              </div>
              ...
              <Button 
                onClick={() => {
                  ...
                  router.push(`/updates/${params.projectId}`)
                }} 
                className="w-full"
              >
                View Updates

...
export function ProjectCard({ project, userVote, onVote, showPromoted = true }: ProjectCardProps) {
  ...
  // Calculate project points (StartUpPush)
  const projectPoints = (voteCounts.upvotes * 10) - (voteCounts.downvotes * 5)

  const handleVote = async (value: number, event?: React.MouseEvent) => {
    console.log('🔥 VOTE BUTTON CLICKED!', { projectId: project.id, value, currentUserVote })
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (errorData.error && errorData.error.includes('need at least 1 StartUpPush point')) {
          ...
          setAlertMessage('You have already voted on this project. Votes are permanent and cannot be changed.')
          setShowAlert(true)
        }
        return
      }
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="flex items-center space-x-2">
                      <div className="flex items-center space-x-1 text-sm font-medium text-purple-600 dark:text-purple-400">
                        <span>🔥</span>
                        <span>{formatNumber(projectPoints)}</span>
